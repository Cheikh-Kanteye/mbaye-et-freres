...
export async function deleteFamille(req: Request) {
  try {
    ...
    await prisma.familles.delete({
      where: { id: idNumber },
    });

    return NextResponse.json(
      { message: "Familles et images produits supprimées avec succès" },
      { status: 200 }
    );
  } catch (error: any) {
    console.error("Error during famille deletion:", error.message);
    return NextResponse.json(
